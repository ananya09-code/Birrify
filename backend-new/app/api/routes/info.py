...
def get_info(db: Session = Depends(get_db)):
    codes = [code.upper() for (code,) in db.query(func.distinct(Rate.currency_code)).order_by(Rate.currency_code)]
    currencies = [BASE_CURRENCY] + [currency_metadata(code) for code in codes if code != "ETB"]
    banks = [{"id": bank.id, "name": bank.name, "short_name": bank.name} for bank in db.query(Bank).order_by(Bank.name)]
